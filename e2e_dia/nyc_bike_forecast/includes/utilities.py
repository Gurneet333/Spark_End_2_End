...
# Utility functions to retreive data from raw sources
def get_bike_data_files():
  spark.sparkContext.addFile(BIKE_FILE_LIST_URL)
  df=spark.read.csv("file://"+SparkFiles.get(BIKE_FILE_LIST_URL.split("/")[-1]), header=True, inferSchema= True)
  return df.toPandas()
...
def get_bike_station_status():
  ...
  if len(df)==0:
    ...
    df=pd.DataFrame([])
  return pd.json_normalize(df['data']['stations'])
...
# utility function to register a hive metatable definition for a specific delta table.
...
def make_verif(forecast, data_train, data_test): 
    ...
    -------
    forecast : 
        The forecast DataFrame including the original observed data.
    """
...
def plot_joint_plot(verif, x='yhat', y='y', title=None, fpath = '../figures/paper', fname = None): 
    ...
    ax.set_xlim([verif['yhat'].min(), verif['yhat'].max()])
    ax.set_ylim([verif['y'].min(), verif['y'].max()])
