...
# MAGIC %md ##Forecasting Using Decision Forests & Temporal Features Only
...
# MAGIC %md Now we define our function.  As with the last notebook, the incoming DataFrame will contain both historical values and future (predicted) values. The function will need to exclude the future values for training purposes and then leverage the full dataset for prediction purposes:
...
def get_forecast(keys, group_pd):
  ...
  model_path = '/dbfs/mnt/citibike/regression_timeonly/{0}'.format(station_id)
...
# MAGIC %md In the function that we'll use to train our models and generate forecasts, we employ a random forest regressor.  As implemented by SciKit-Learn, the RandomForestRegressor model generates a prediction but does not provide a prediction interval.  Leveraging a modified version of a function provided at https://blog.datadive.net/prediction-intervals-for-random-forests/, we can derive a prediction interval so that our result better aligns with that produced by the Prophet models employed earlier.  This function is called by the get_forecast function defined in the previous cell:
...
# MAGIC %md And now we are ready to assemble our dataset for training and forecasting.  While we will not be employing any timeseries techniques, we will derive some features from the period timestamp.  Hour of day, day of week, month and year will be employed along with a flag indicating whether or not a period is associated with a holiday. 
# MAGIC 
# MAGIC As before, we will employ the *is_historical* flag to seperate historical from future values:
...
forecast.createOrReplaceTempView('forecast_regression_timeonly')
...
# MAGIC %sql
# MAGIC DROP TABLE IF EXISTS citibike.forecast_regression_timeonly;
# MAGIC 
# MAGIC CREATE TABLE citibike.forecast_regression_timeonly
# MAGIC USING DELTA
# MAGIC AS
# MAGIC   SELECT * 
# MAGIC   FROM forecast_regression_timeonly

# COMMAND ----------

# MAGIC %md Visualizing the forecast for Station 518 will take a little more work than last time.  We'll need to recreate the plot logic used by Prophet, modifying it for our dataset:
...
forecast_pd = spark.sql('''
      SELECT
        ...
        a.yhat_lower,
        a.yhat_upper
      FROM citibike.forecast_regression_timeonly a
...
model = mlflow.sklearn.load_model('/dbfs/mnt/citibike/regression_timeonly/518')
...
# MAGIC       a.yhat
# MAGIC     FROM citibike.forecast_regression_timeonly a
